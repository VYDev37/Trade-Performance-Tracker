    ...
    return (
        <div className="space-y-3 md:space-y-4">
            <div className="flex justify-between items-center px-1">
                ...
                <Button variant="ghost" onClick={onViewAll} className={`text-[10px] bg-transparent ${colorClass} hover:underline font-bold uppercase`}>
                    View All
                </Button>
            </div>
                ...
                {items.length > 0 ? items.map((item) => (
                    ...
                    <div className="h-14 border border-dashed border-slate-800/80 rounded-xl flex items-center justify-center">
                        <span className="text-[10px] text-slate-700 font-medium italic">
                            {isIncome ? 'Pending activity...' : 'No recent outflows'}
                        </span>
                    </div>
                )}
            </div>
